...
https://github.com/sgomber/CDCL-SAT.
"""

import time
from collections import OrderedDict

from auxiliary.priority_queue import PriorityQueue
from auxiliary.luby_generator import reset_luby, get_next_luby_number
...
class CDCLSolver:
    ...
    def __init__(self, branching="ORDERED", restart=None, verbose=True):
        ...
        if branching not in ["ORDERED","VSIDS","MINISAT"]:
            raise ValueError(
                'The branching heuristic must be one from the list ' + \
                '["ORDERED","VSIDS","MINISAT"]'
            )
        self._decider = branching

        if not restart:
            self._restarter = None
        else:

            if restart not in ["GEOMETRIC","LUBY"]:
                raise ValueError(
                    'The restarting strategy must be one from the list ' + \
                    '[None, "GEOMETRIC","LUBY"]'
                )
            ...
            while itr < len(clauses_watched_by_falsed_literal):
                ...
                if new_literal_to_watch != -1:
                    ...
                    if other_watch_var not in self._variable_to_assignment_nodes:
                        ...
                        if self._decider == "VSIDS":
                            self._priority_queue.remove(
                                other_watch_var
                            )
                            self._priority_queue.remove(
                                other_watch_var + self._num_vars
                            )
                        ...
                        if self._restarter == "GEOMETRIC":
                            ...
                            if self._conflicts_before_restart >= \
                                ...
                                if self._verbose:
                                    print("RESTARTING with GEOMETRIC " + \
                                    f"RESTART LIMIT {self._conflict_limit}")
                        ...
                        if self._restarter == "LUBY":
                            ...
                            if self._conflicts_before_restart >= \
                                ...
                                self.stats._restarts += 1
                                self._conflicts_before_restart = 0
                                self._conflict_limit = self._luby_base * \
                                    get_next_luby_number()

                                if self._verbose:
                                    print("RESTARTING with LUBY RESTART " + \
                                    f"LIMIT {self._conflict_limit}")
    ...
    def _analyze_conflict(self):
        ...
        if self._verbose:
            print(
                "= \x1b[31mCONFLICT!\x1b[0m Learned clause:\x1b[35m",
        ...
        if len(conflict_clause) > 1:
            ...
            if self._verbose:
                print(f"\x1b[34m< Backjumping to level " + \
    ...
    def _backtrack(self, backtrack_level, node_to_add):
        ...
        while True:
            ...
            if self._assignment_stack[itr].level <= backtrack_level:
                ...
                if self._decider == "VSIDS":
                    self._priority_queue.add(
                        node.var,
                        self._lit_scores[node.var]
                    )
                    self._priority_queue.add(
                        node.var + self._num_vars,
                        self._lit_scores[node.var + self._num_vars]
                    )

                if self._decider == "MINISAT":
                    self._priority_queue.add(
                        node.var,
                        self._var_scores[node.var]
                    )
        ...
        if node_to_add:
            ...
            if self._decider == "VSIDS":
                ...
                self._priority_queue.remove(
                    node_to_add.var + self._num_vars
                )
